<script lang="ts">
  ...
  import type { Segment, SegmentRule } from "$lib/types.js";
  ...
  let formData = $state({
    ...
    name: "",
    description: "",
    rules: [] as { attribute: string; operator: string; value: string }[],
  });
  ...
  function resetForm() {
    formData = {
      ...
      name: "",
      description: "",
      rules: [{ attribute: "", operator: "equals", value: "" }],
    };
  ...
  function addRule() {
    formData.rules.push({ attribute: "", operator: "equals", value: "" });
  ...
  async function handleSubmit(e: Event) {
    ...
    const validRules = formData.rules.filter(
      (rule) => rule.attribute.trim() && rule.value !== "",
    );

    if (validRules.length === 0) {
      error = "Please add at least one rule with an attribute and value";
      return;
    }
  ...
  function formatRules(rules: SegmentRule[]): string {
    return rules
      .filter((rule) => rule.attribute && rule.value != null)
      .map(
        (rule) =>
          `${rule.attribute} ${operatorLabels[rule.operator] || rule.operator} ${rule.value}`,
      )
      .join(" AND ");
  }
          ...
          <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
            ...
            <div>
              ...
              <input
                ...
                class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm {editingSegment
                  ? 'bg-gray-50'
                  : ''}"
                placeholder="beta-users"
              />
            ...
            <div class="space-y-4">
              {#each formData.rules as rule, ruleIndex (ruleIndex)}
                ...
                {#if ruleIndex < formData.rules.length - 1}
                  <div
                    class="text-center text-sm text-gray-500 font-medium py-2"
                  >
                    OR
                  </div>
                {/if}
            ...
            <div class="flex items-center justify-between">
              <div class="flex-1">
                <div class="flex items-center space-x-2">
                  ...
                  >
                    <svg
                      class="w-5 h-5 transform {expandedSegments.has(segment.id)
                        ? 'rotate-180'
                        : ''}"
                      fill="none"
                      stroke="currentColor"
                      ...
                      <div class="text-sm text-gray-600">
                        ...
                        <span class="mx-1"
                          >{operatorLabels[rule.operator] ||
                            rule.operator}</span
                        >
                        <code class="bg-white px-1 rounded">{rule.value}</code>
                      </div>
                    </div>
                    {#if ruleIndex < segment.rules.length - 1}
                      <div
                        class="text-center text-sm text-blue-600 font-medium"
                      >
                        OR
                      </div>
                    {/if}
